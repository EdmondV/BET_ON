...
We've found that for many applications treating single pages (e.g. the Loginpage, the LoginPage etc.) as containers and their small parts (e.g. the Login form, the Navigation bar) components works well, but there are no rigid rules. **Bend the architecture to the needs of your app, nothing is set in stone!**
...
- `ReactDOM.render()` not only renders the [root react component](https://github.com/react-boilerplate/react-boilerplate/blob/master/app/containers/App/TutorModal.js) called `<App />`, of your application, but it renders it with `<Provider />`, `<LanguageProvider />` and `<Router />`.
...
- Path `"/"` corresponds to container `<LoginPage />`
...
**When you navigate to `"/"`, only files related to the LoginPage will be downloaded and subsequently executed. This makes your application incredibly lightweight and lightning fast.**
...
Run `npm start` to launch the application. If you start browsing at [https://localhost:3000](https://localhost:3000), by default you will be navigated to the home page. Here, notice that route is `"/"`, so the [`<LoginPage />`](https://github.com/react-boilerplate/react-boilerplate/blob/master/app/containers/LoginPage/TutorModal.js) container will be mounted. It is responsible for rendering a form with a textbox and a list of repositories.
...
Check out [`LoginPage/sagas.js`](https://github.com/react-boilerplate/react-boilerplate/blob/master/app/containers/LoginPage/sagas.js). It can be confusing for untrained eyes. The API of `redux-saga` is self-descriptive once you've seen it, so let's go over what happens in there:
...
_An update has occurred!_ `mapStateToProps()` will be triggered. `<LoginPage />` will receive the new data and rerender.
