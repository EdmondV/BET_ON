...
All default English text for the component is contained here (e.g. `This project is licensed under the MIT license.`), and is tagged with an ID (e.g. `Footer.license.message`) in addition to it's object definition id (e.g. `licenseMessage`).
